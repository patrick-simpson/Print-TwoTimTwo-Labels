(function() {
  ...
  // ── Centralized TwoTimTwo DOM selectors ──────────────────────────────────
  // When TwoTimTwo redesigns their check-in page, THIS is the single file
  // and the single block to edit. Every selector used below is defined here
  // so future breakage has one place to look.
  const SELECTORS = {
    LAST_CHECKIN:      '#lastCheckin',          // wrapper whose child changes on local check-in
    LAST_CHECKIN_INNER:'#lastCheckin div',      // the element holding the checked-in child's name
    CLUBBER:           '.clubber',              // each visible child card in the roster list
    CLUBBER_NAME:      '.name',                 // name element within .clubber
    CLUB_IMAGE:        '.club img',             // club logo image within .clubber
    CHECKIN_MODAL:     '#checkin-modal',        // TwoTimTwo's confirm-checkin modal
    CHECKIN_BUTTON:    'button#checkin',        // the confirm button inside that modal
  };
  ...
  // ── Selector heartbeat ────────────────────────────────────────────────────
  // If TwoTimTwo ever redesigns the check-in page, .clubber / #lastCheckin
  // may stop matching anything and the extension silently stops working.
  // Every 60 s we verify those selectors still resolve; after three
  // consecutive failures on a page that *should* have them, log a clear
  // warning so the operator has something actionable to show in support.
  var selectorFailCount = 0;
  setInterval(function() {
    try {
      // Only raise an alert on pages that look like the check-in page.
      if (!/clubber\/checkin/i.test(location.pathname)) return;
      var lastCheckinEl = document.querySelector(SELECTORS.LAST_CHECKIN);
      var clubberEls    = document.querySelectorAll(SELECTORS.CLUBBER);
      if (!lastCheckinEl || clubberEls.length === 0) {
        selectorFailCount++;
        if (selectorFailCount === 3) {
          console.warn(
            '[Awana] Heartbeat: expected check-in selectors are no longer matching. ' +
            'TwoTimTwo may have changed their page structure. ' +
            'Edit SELECTORS at the top of extension/content.js and rerun. ' +
            'Details: lastCheckin=' + (!!lastCheckinEl) + ', clubbers=' + clubberEls.length
          );
        }
      } else {
        selectorFailCount = 0;
      }
    } catch (e) { /* never let heartbeat crash the observer */ }
  }, 60 * 1000);

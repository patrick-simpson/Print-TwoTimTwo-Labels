...
# The browser extension will sync fresh data from the authenticated browser session anyway.
$hasRealCsv = $false
if (Test-Path $clubbersCsvPath) {
    ...
    if ($csvLines.Count -gt 4) {
        ...
        Write-Host "  The browser extension will sync fresh data when you open the check-in page." -ForegroundColor Gray
    }
}
...
if (-not $hasRealCsv) {
    ...
    if (-not $downloaded) {
        Write-Host "  No roster data yet -- the browser extension will sync it from your browser." -ForegroundColor Yellow
...
Write-Host "  1. Install the browser extension (chrome-extension/)" -ForegroundColor Gray
Write-Host "  2. Go to the check-in page — the extension will detect check-ins" -ForegroundColor Gray
Write-Host "  3. Labels print automatically to your label printer" -ForegroundColor Gray
Write-Host ""
# Open the check-in page in Edge.
Write-Host "Opening check-in page in default browser..." -ForegroundColor Cyan
Start-Process $cfg.checkinUrl
Write-Host ""
Write-Host "  Make sure you have the browser extension installed for auto-printing." -ForegroundColor Yellow
Write-Host "  The extension provides zero-click labels directly to your printer." -ForegroundColor Yellow

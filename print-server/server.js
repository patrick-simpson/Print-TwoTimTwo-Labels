...
// ── Timestamp all log output ──────────────────────────────────────────────────
const _origLog   = console.log.bind(console);
const _origWarn  = console.warn.bind(console);
const _origError = console.error.bind(console);
console.log   = (...a) => _origLog(  new Date().toISOString(), ...a);
console.warn  = (...a) => _origWarn( new Date().toISOString(), ...a);
console.error = (...a) => _origError(new Date().toISOString(), ...a);
...
// ── Pure utility functions (CSV parsing, birthday check, allergy detection) ───
const { HEADER_MAP, ALLERGY_EMOJI, normalizeHeader, parseCSV, isBirthdayWeek, parseAllergies } = require('./utils');
...
// given font/size. Prevents text from printing off the edge of the label.
...
`.trim();
  ...
  try {
    ...
    fs.unlink(psPath, (err) => { if (err) console.warn('[cleanup] Failed to unlink temp PS1:', err.message); });
  }
}
...
// ── Receive CSV from the extension (authenticated browser session) ───────────
// The extension fetches /clubber/csv from the same origin (which has the
...
app.post('/print', async (req, res) => {
  ...
  if (firstName.length > 100 || lastName.length > 100) {
    return res.status(400).json({ error: 'Name fields exceed maximum length (100 chars)' });
  }
  const clubNameStr = String(clubName || '').trim().slice(0, 100);
  if (clubImageData && typeof clubImageData === 'string' && clubImageData.length > 5 * 1024 * 1024) {
    return res.status(400).json({ error: 'clubImageData too large (max 5 MB)' });
  }
  ...
  try {
    ...
    if (pngPath) fs.unlink(pngPath, (err) => { if (err) console.warn('[cleanup] Failed to unlink temp PNG:', err.message); });
  }
});

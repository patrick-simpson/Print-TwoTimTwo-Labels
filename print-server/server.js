...
async function generateLabel(
  ...
  if (weatherTheme) {
    ctx.save();
    ctx.lineWidth = 10;
    ctx.strokeStyle = weatherTheme.color;
    // Draw a thick border around the badge (with 2pt extra margin)
    roundedRect(ctx, BX - 2, BY - 2, BW + 4, BH + 4, CORNER + 2);
    ctx.stroke();
    
    // Draw emojis in corners
    ctx.font = '24px "Segoe UI Emoji", "Apple Color Emoji", "Noto Color Emoji", sans-serif';
    ctx.textBaseline = 'middle';
    ctx.textAlign = 'center';
    const e = weatherTheme.emoji;
    ctx.fillText(e, BX + 15, BY + 15);
    ctx.fillText(e, BX + BW - 15, BY + 15);
    ctx.fillText(e, BX + 15, BY + BH - 15);
    ctx.fillText(e, BX + BW - 15, BY + BH - 15);
    ctx.restore();
  }
...
app.post('/print', async (req, res) => {
  ...
  if (totalPrintsCount % 10 === 0) {
    console.log(`[weather] Print #${totalPrintsCount} - Fetching weather banner...`);
  ...
  try {
    ...
    pngPath = await generateLabel(
      firstName, lastName, clubName, clubImageBuffer,
      allergyTokens, handbookGroup, birthday, weatherTheme
    );
